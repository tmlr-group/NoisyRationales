...
class generate_test:
    def __init__(self, args) -> None:
        ...
        self._if_in_context = generate_config["if_in_context"] if "if_in_context" in generate_config else False
        if self._if_in_context:
            self._if_noise = generate_config["if_noise"] if "if_noise" in generate_config else False
            self._n_shots = generate_config["n_shots"] if "n_shots" in generate_config else 1
            self._n_weak_shots = generate_config["n_weak_shots"] if "n_weak_shots" in generate_config else 0
        else:
            self._if_noise = False
            self._n_shots = 0
            self._n_weak_shots = 0
        if self._if_noise:
            self._n_noisy_shots = generate_config["n_noisy_shots"] if "n_noisy_shots" in generate_config else 0
        ...
        if set_file_path is None:
            ...
            if self._if_noise == False:
                ...
                if self._noise_ratio == 0.3: 
                    ...
                    raise ValueError(f"noise ratio {self._noise_ratio} not support in default path")
